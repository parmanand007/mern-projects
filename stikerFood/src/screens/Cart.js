  ...
  return (
      ...
      <div className='container m-auto mt-5 table-responsive-sm table-responsive-md'>
        ...
        {data.map((food, index) => (
              ...
              <tr className='text-white '>
                ...
                <td ><button type="button" className="btn p-0">
                    <AiFillDelete className='text-white'  onClick={() => { dispatch({ type: "REMOVE", index: index }) }} />
                    </button> </td></tr>
            ))}
        </tbody>
       </table>
       ...
       <div><h1 className=" text-white fs-2">Total Price :{totalPrice}/-</h1></div>
        <div><button className='btn btn-primary mt-5' onClick={handleCheckout}>Check Out</button></div>
      </div>
    </div>
  )
}
